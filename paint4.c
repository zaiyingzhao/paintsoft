...
typedef enum res{ EXIT, LINE, RECT, CIRCLE, LOAD, INVALIDFILE, CHPEN, VISIONTEST, UNDO, SAVE, UNKNOWN, ERRNONINT, ERRLACKARGS} Result;
...
void chpen(Canvas *c, char p);
void visiontest(Canvas *c, char *size, char *direction);
...
int main(int argc, char **argv)
{
	...
	printf("%s\n",strresult(r));
	// History構造体に入れる
	if (r == LINE || r == RECT || r == CIRCLE || r == LOAD || r == CHPEN) {
...
Result interpret_command(const char *command, History *his, Canvas *c)
{
    ...
    if (strcmp(s, "visiontest") == 0) {
        char *size = strtok(NULL, " ");
        char *direction = strtok(NULL, " ");
        visiontest(c, size, direction);
        return VISIONTEST;
    }
...
char *strresult(Result res){
    ...
    return "pen changed";
    case VISIONTEST:
    return "ask patient which direction the gap exists in the circle";
    case UNDO:
	return "undo!";
    case UNKNOWN:
